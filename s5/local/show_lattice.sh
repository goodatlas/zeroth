#!/bin/bash
# Modified for using Korean font 
# sudo apt-get install fonts-nanum  
#
# 2018	Atlas Guide (Lucas Jo)
...
fstdraw --portrait=true --osymbols=$words $tmpdir/$uttid.fst |\
	sed -E 's/fontsize/fontname\=\"NanumMyeongjo.ttf\" fontsize/g' | dot -T${format} > $tmpdir/$uttid.${format}
